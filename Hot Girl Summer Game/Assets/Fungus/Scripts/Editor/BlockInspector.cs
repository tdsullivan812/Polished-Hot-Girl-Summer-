...
namespace Fungus.EditorUtils
{
    ...
    public class BlockInspectorEditor : Editor
    {
        ...
        private void ResizeScrollView(Flowchart flowchart)
        {
            ...
            if (cursorChangeRect.Contains(UnityEngine.Event.current.mousePosition))
            {
            ...
            if (resize && UnityEngine.Event.current.type == EventType.Repaint)
            {
                Undo.RecordObject(flowchart, "Resize view");
                flowchart.BlockViewHeight = UnityEngine.Event.current.mousePosition.y - topPanelHeight;
            }
            ...
            if (resize && UnityEngine.Event.current.type == EventType.MouseDrag)
            {
        ...
        protected virtual void ClampBlockViewHeight(Flowchart flowchart)
        {
            ...
            if (UnityEngine.Event.current.commandName != "")
            {
                clamp = false;
            }
