    ...
    public class CommandListAdaptor
    {
        ...
        public void DrawItem(Rect position, int index, bool selected, bool focused)
        {
            ...
            if (UnityEngine.Event.current.type == EventType.MouseDown &&
                ...
                clickRect.Contains(UnityEngine.Event.current.mousePosition))
            {
                if (flowchart.SelectedCommands.Contains(command) && UnityEngine.Event.current.button == 0)
                {
                    // Left click on already selected command
                    // Command key and shift key is not pressed
                    if (!EditorGUI.actionKey && !UnityEngine.Event.current.shift)
                    {
                ...
                {
                    bool shift = UnityEngine.Event.current.shift;

                    // Left click and no command key
                    if (!shift && !EditorGUI.actionKey && UnityEngine.Event.current.button == 0)
                    {
                        BlockEditor.actionList.Add(delegate
                        {
                            flowchart.ClearSelectedCommands();
